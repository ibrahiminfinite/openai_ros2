...
class Bullet(object):
    ...
    def step(self):
        if self.time_step <= 1 / 240:
            num_sim_steps = 0.01 / (self.time_step)
        else:
            raise Exception(' Simulator update rate less than controller update rate (Non integer num steps not supported')

        for i in range(int(num_sim_steps)):
    ...
    def reset(self):
        # TODO : reset world. (does not apper to be required at the moment)
        for joint in self.motor_joints:
